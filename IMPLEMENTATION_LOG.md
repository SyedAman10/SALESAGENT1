...
| **Registrant relay leads** — when a domain-variant holder is privacy-protected (no scrapeable contact), the agent does everything except the final click: RDAP enrichment (registrar, holding period), the registrar's contact-relay link, and a drafted message — packaged in the daily report as a ~60-second manual task. Form submission stays manual by design: automating registrar relay forms violates their ToS and risks abuse complaints. First run produced a ready-to-send lead (indika.club, held since 2022) | ✅ Live (2026-06-12) |
| **Reddit "want to buy" discovery** — Reddit blocks direct scraping outright (verified: datacenter AND residential proxies both 403). Activated via Google SERP discovery instead: WTB posts about domain names are found through search, intent-filtered, and queued as manual DM tasks in the daily report. Honest expectation: a trickle channel (~0–2 qualified posts/week), at zero marginal effort once automated | ✅ Live via SERP (2026-06-12) |
